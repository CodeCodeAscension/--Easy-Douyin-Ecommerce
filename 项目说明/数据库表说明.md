...
-- ----------------------------
-- Table structure for addressInfoVo
-- ----------------------------
DROP TABLE IF EXISTS `addressInfoVo`;
CREATE TABLE `addressInfoVo`  (
...
CREATE TABLE `order`  (
  ...
  CONSTRAINT `fk_address_id` FOREIGN KEY (`address_id`) REFERENCES `addressInfoVo` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE
