...
经过网关测试需要用户登录的接口，请求头设置`Authorization`字段，不经过网关直接测试微服务接口，请求头设置`X-User-Id`和`X-User-Power`字段。

所有接口都会返回`ResponseResult`对象，形式为：
...
接口有`需要用户登录`和`需要管理员登陆`这两种说明，没有出现说明则默认为`不需要用户登录`。
...
```json
{
    ...
    "data": {
        ...
        "accessToken": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJjbGFpbXMiOnsiaXNSZWZyZXNoVG9rZW4iOmZhbHNlLCJ1c2VySWQiOjE4OTIyMjU3MjIwNDc3MTMyODEsInVzZXJQb3dlciI6MX0sImV4cCI6MTc0MTEwMzQ3N30.7FaNcxW3C0dYtiT-4tOcY-MNGMWJqkcUdr0WTCSRfug",
        "refreshToken": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJjbGFpbXMiOnsiaXNSZWZyZXNoVG9rZW4iOnRydWUsInVzZXJJZCI6MTg5MjIyNTcyMjA0NzcxMzI4MSwidXNlclBvd2VyIjoxfSwiZXhwIjoxNzQxNzA0Njc3fQ.4atExsh9BYL8IvQMilczCQBAm7utI30PCQttlqu56g4"
    }
}
```
...
|      stock       |       商品库存       | createProductDto |  false   |     Integer      |
...
|    addressInfoVo    |    用户收获地址    | placeOrderDto | true  |     Address      |
| streetAddress |     街道地址     |    addressInfoVo    | true  |      String      |
|     city      |      城市      |    addressInfoVo    | true  |      String      |
|   province    |      省份      |    addressInfoVo    | true  |      String      |
|    country    |      国家      |    addressInfoVo    | true  |      String      |
|    zipCode    |     邮政编码     |    addressInfoVo    | true  |      String      |
...
|    addressInfoVo     |            用户收获地址            | updateOrderDto | false |     Address      |
| streetAddress  |             街道地址             |    addressInfoVo     | false |      String      |
|      city      |              城市              |    addressInfoVo     | false |      String      |
|    province    |              省份              |    addressInfoVo     | false |      String      |
|    country     |              国家              |    addressInfoVo     | false |      String      |
|    zipCode     |             邮政编码             |    addressInfoVo     | false |      String      |
...
|     userCurrency      |              用户使用的货币               |      String      |
|        addressInfoVo        |               用户收获地址               |     Address      |
| addressInfoVo.streetAddress |                街道地址                |      String      |
|     addressInfoVo.city      |                 城市                 |      String      |
|   addressInfoVo.province    |                 省份                 |      String      |
|    addressInfoVo.country    |                 国家                 |      String      |
|    addressInfoVo.zipCode    |                邮政编码                |      String      |
|         email         |                用户邮箱                |      String      |
...
|   orderStatusEnum   | 	订单状态（0待确认，1已确认待支付，2已支付，3已取消）	 | 	aiOrderQueryDto | 	false	 |     Integer     |
