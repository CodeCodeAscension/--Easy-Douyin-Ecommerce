...
|       参数名称       |    参数说明     |       传参位置       | 是否必须  |      数据类型      |
|:----------------:|:-----------:|:----------------:|:-----:|:--------------:| 
| updateProductDto |   创建的商品信息   |   @RequestBody   | true  |   ProductDto   | 
...
|     status     | 支付状态（0待支付，1已支付，2已取消） | updateOrderDto | true  |     Integer      |
...
**请求参数**：`@RequestBody CancelOrderDto`

|        参数名称        |         参数说明         |      传参位置      | 是否必须 |      数据类型      |
|:------------------:|:--------------------:|:--------------:|:----:|:--------------:| 
|   cancelOrderDto   |       订单定时取消信息       |  @RequestBody  | true | CancelOrderDto |
|      orderId       |         订单ID         | cancelOrderDto | true |     String     |
|       status       | 是否设置定时取消（可以取消上次的定时）  | cancelOrderDto | true |    Boolean     |
| cancelAfterMinutes | 订单过多少分钟后自动取消（0为立即取消） | cancelOrderDto | true |    Integer     |
...
**返回对象**：`ResponseResult<List<OrderInfoVo>>`，其中`data`中每一个`OrderInfoVo`字段的定义为：

|         参数名称          |         参数说明         |       数据类型       |
|:---------------------:|:--------------------:|:----------------:|
|        orderId        |         订单ID         |      String      |
|        status         | 支付状态（0待支付，1已支付，2已取消） |     Integer      |
|     userCurrency      |       用户使用的货币        |      String      |
...
**接口地址**：`/api/v1/orders/paid`

**请求方式**：`POST`

**请求数据类型**：`application/json`

**请求参数**：`@RequestBody MarkOrderPaidDto`

|       参数名称       |   参数说明    |       传参位置       | 是否必须 |       数据类型       |
|:----------------:|:---------:|:----------------:|:----:|:----------------:| 
| markOrderPaidDto | 设置订单已支付信息 |   @RequestBody   | true | MarkOrderPaidDto |
|     orderId      |   订单ID    | markOrderPaidDto | true |      String      |
...
|  amount   |  交易金额  |  chargeDto   | true |   Float   |
...
**请求参数**：`@RequestParam transactionId`
...
|   transactionId    |         交易ID         | chargeCancelDto | true |     String      |
...
|    queryType    |  	查询类型（0自然语言，1JSON条件）  | 	aiOrderQueryDto |  	true  |    	Integer     |
|  queryContent   |  	查询内容（自然语言文本/JSON条件）  | 	aiOrderQueryDto |  	true  |     	String     |
|   orderStatus   | 	订单状态（0待支付，1已支付，2已取消）	 | 	aiOrderQueryDto | 	false	 |     Integer     |
...
|   triggerType   | 触发类型（0根据历史订单信息，1根据自然语言指令要求） | aiPlaceOrderDto |      true      |     Integer     |
|   historyDays   |          历史订单参考天数           | aiPlaceOrderDto | triggerType==0 |     Integer     |
|     command     |           自然语言指令            | aiPlaceOrderDto | triggerType==1 |     Integer     |
...
|   参数名称    |     参数说明      |  数据类型  |
|:---------:|:-------------:|:------:|
|  orderId  | 订单ID（无需确认时返回） | String |
| confirmId | 确认ID（需要确认时返回） |  Long  |
